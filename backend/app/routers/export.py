...
async def export_status(task_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    return ok(export_service.get_task_status(task_id))


@router.get("/database/download/{task_id}")
async def export_download(task_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):

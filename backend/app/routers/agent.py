...
@router.post("/confirm")
async def confirm(data: AgentConfirmRequest, request: Request, db: AsyncSession = Depends(get_db), user: User = Depends(require_permission("agent"))):
    result = await agent_service.agent_confirm(db, data.execution_id, data.confirmed, get_client_ip(request))
    return ok(result)


@router.post("/explain")
async def explain(data: ExplainRequest, request: Request, db: AsyncSession = Depends(get_db), user: User = Depends(require_permission("agent"))):
